...
df.clim <- get_weather(env.id = env, lat = lat, lon = lon, asdataframe = T,
...
W <- W.matrix(df.cov = df.clim, by.interval = F)
dim(W)
W[,1:5]

## same as SummaryWTH, we can add time.windows
W <- W.matrix(df.cov = df.clim, by.interval = T, 
         time.window = c(0, 14, 35, 60, 90, 120))
dim(W)
W[,1:5]

## and select the statistic to be used
W <- W.matrix(df.cov = df.clim, by.interval = T, statistic = 'mean',
         time.window = c(0, 14, 35, 60, 90, 120))
dim(W)
W[,1:5]

W <- W.matrix(df.cov = df.clim, by.interval = T, statistic = 'quantile',
         time.window = c(0, 14, 35, 60, 90, 120))
dim(W)
W[,1:5]

## We can perform a Quality Control (QC) based on the maximum sd tolered
W <- W.matrix(df.cov = df.clim, by.interval = F, QC = T)
dim(W)
W[,1:5]

## We can perform a Quality Control (QC) based on the maximum sd tolered
W <- W.matrix(df.cov = df.clim, by.interval = F, QC = T, sd.tol = 3)
dim(W)
W[,1:5]

## or create W for specific variables
id.var = c('T2M_MAX','T2M_MIN','T2M')
W <- W.matrix(df.cov = df.clim, var.id = id.var)
dim(W)
W[,1:3]

## or combine with summaryWTH by using is.processed = T
data <- summaryWTH(df.clim, env.id = 'env', statistic = 'quantile')
W <- W.matrix(df.cov = data, is.processed = T)
dim(W)
W[,1:3]
...
# obtaining the benchmark main effect model: Y = fixed + G
MM <- get_kernel(K_G = list(G = as.matrix(maizeG)),Y = maizeYield, reaction = F, model = 'MM')

# or the benchmark main GxE deviation model: Y = fixed + G +GE
MDs <- get_kernel(K_G = list(G = as.matrix(maizeG)),Y = maizeYield, reaction = F, model = 'MDs')


# Creating the envirotyping covariance matrix based on phenology time intervals
W.cov <- W.matrix(df.cov = maizeWTH,by.interval = T, statistic = 'quantile',
                  time.window = c(0, 14, 35, 60, 90, 120))
dim(W.cov)
W.cov[,1:4]
...
H <- EnvKernel(df.cov = W.cov, Y = maizeYield, merge = T, env.id = 'env')
class(H)
...
W.cov <- W.matrix(df.cov = maizeWTH, 
                  by.interval = T, 
                  statistic = 'quantile',
                  time.window = c(0, 14, 35, 60, 90, 120))
...
EMM <-get_kernel(K_G = list(G=G),K_E = list(W=W$envCov,eT=eT$envCov), 
                 Y = Y,model = 'EMM',size_E = 'environment')
